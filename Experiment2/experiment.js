...
var MAX_POINTS=22,incident1,incident2,incident3,refracted1,refracted2,virtual1,virtual2,drawC;
var lens;
var MAX_POINTS2=32;
var object,objectAlphabet;
var posxi3,posyi3;
var posxi2,posyi2;
var posv1x,posv1y;
var posv2x,posv2y;
...
var count=0;
drawC=1;
var string1=["When you increase the object distance","(less than focal length)","Image distance increases","also the image size"];
var string2=["When you increase the focal length","(less than focal length)","Image distance decreases with ","the increase of focal length","for a constant object distance"];
var virtual_learn;
...
function initializeScene()
{
    ...
    loadOpticalbench();
    objectAlphabet="A";
    PIEaddInputText("Analphabet :","A",objectAlphabetHandler);
    initializeObject();
    PIEscene.background=new THREE.Color(0x010544);
    var groundMaterial = new THREE.MeshPhongMaterial( { color: 0x330101, specular: 0x111111} );
...
function objectAlphabetHandler(value)
{
    objectAlphabet=value;
    PIEremoveElement(objecthandle);
    initializeObject();
}
function loadOpticalbench()
{
    var opticalbench=new THREE.Mesh(new THREE.BoxGeometry(4,1.5,15),new THREE.MeshStandardMaterial({color:0x083E0A,emissive:0x031604,metalness:1}));
    PIEaddElement(opticalbench);


    var cylinderbar1=new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.1,1,32),new THREE.MeshStandardMaterial({color:0x013F04,emissive:0x4E7A7A,metalness:1}));
    opticalbench.add(cylinderbar1);
    cylinderbar1.position.y=1;
    cylinderbar1.position.z=-7.3;
    cylinderbar1.position.x=-1.7;


    var cylinderbar2=new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.1,1,32),new THREE.MeshStandardMaterial({color:0x013F04,emissive:0x4E7A7A,metalness:1}));
    opticalbench.add(cylinderbar2);
    cylinderbar2.position.y=1;
    cylinderbar2.position.z=-7.3;
    cylinderbar2.position.x=1.7;

    var cylinderbar3=new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.1,1,32),new THREE.MeshStandardMaterial({color:0x013F04,emissive:0x4E7A7A,metalness:1}));
    opticalbench.add(cylinderbar3);
    cylinderbar3.position.y=1;
    cylinderbar3.position.z=7.3;
    cylinderbar3.position.x=1.7;
    ...
    var cylinderbar4=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,15,32),new THREE.MeshStandardMaterial({color:0x013F04,emissive:0x4E7A7A,metalness:1}));
    opticalbench.add(cylinderbar4);
    cylinderbar4.rotation.x=Math.PI/2
    cylinderbar4.position.y=1.5;
    cylinderbar4.position.x=-1.7;

    var cylinderbar5=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,15,32),new THREE.MeshStandardMaterial({color:0x013F04,emissive:0x4E7A7A,metalness:1}));
    opticalbench.add(cylinderbar5);
    cylinderbar5.rotation.x=Math.PI/2
    cylinderbar5.position.y=1.5;
    cylinderbar5.position.x=1.7;


    opticalbench.rotation.y=Math.PI/2;

    opticalbench.position.y=-6.2

    opticalbench.scale.z=1.6;
    opticalbench.scale.x=0.6;
    PIErender();
}
function loadtTable()
{
    var backtexture=new THREE.TextureLoader();
    backtexture.load("hardwood2_diffuse.jpg",function (texture)
    {
        var material = new THREE.MeshBasicMaterial( { map: texture ,transparent: false} );
        table=new THREE.Mesh(new THREE.BoxGeometry(35,10,1),material);
        //alert("loader")
        table.position.y=-7;
        table.rotation.x+=Math.PI/2;
        var table_leg1,table_leg2,table_leg3,table_leg4;


        table_leg1=new THREE.Mesh(new THREE.BoxGeometry(1,1,10),material);
        table.add(table_leg1);
        table_leg1.position.x=15;
        table_leg1.position.z=4.5;
        table_leg1.position.y=4.5;

        table_leg2=new THREE.Mesh(new THREE.BoxGeometry(1,1,10),material);
        table.add(table_leg2);
        table_leg2.position.x=-15;
        table_leg2.position.z=4.5;
        table_leg2.position.y=4.5;

        table_leg3=new THREE.Mesh(new THREE.BoxGeometry(1,1,10),material);
        table.add(table_leg3);
        table_leg3.position.x=-15;
        table_leg3.position.z=4.5;
        table_leg3.position.y=-4.5;

        table_leg4=new THREE.Mesh(new THREE.BoxGeometry(1,1,10),material);
        table.add(table_leg4);
        table_leg4.position.x=15;
        table_leg4.position.z=4.5;
        table_leg4.position.y=-4.5;
...
function initializeObject()
{
    ...
    object=new THREE.Mesh(new THREE.TextGeometry(objectAlphabet,{font:font,size:3,height:.1,material:0}),new THREE.MeshLambertMaterial({color:0XFB350B  }));
    object.rotation.y+=Math.PI/2;

    objecthandle=new THREE.Mesh(new THREE.CylinderGeometry(0.25,0.25,2,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
    var cylinder1=new THREE.Mesh(new THREE.CylinderGeometry(0.25,0.25,2,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
    objecthandle.add(cylinder1);
    cylinder1.rotation.x=Math.PI/2;
    cylinder1.position.y=-1;

    var cylinder2=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,1.5,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
    objecthandle.add(cylinder2);
    cylinder2.position.z=-1;
    cylinder2.position.y=-1.5;

    var cylinder3=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,1.5,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
    objecthandle.add(cylinder3);
    cylinder3.position.z=1;
    cylinder3.position.y=-1.5;

    objecthandle.position.y=-2.5;
    objecthandle.position.z=0;
    objecthandle.add(object);
    object.position.y+=1;
    object.position.z+=1.5;
    PIEaddElement(objecthandle);
}
var virtualObj,height;
var firstheight=true;
var first_learn=true;
function calculateHeight()
{
    if(firstheight)
    {
        ...
        if(iy>a)
            ...
            height=(a/iy+a%iy);
        //consolee.log("height :"+height+"  iy: "+iy+" y:"+a+"first:"+firstheight);
        firstheight=false;
    }
    else
    {
        ...
        if(iy>a)
            ...
            height=(a/iy+a%iy);
        //consolee.log("height :"+height+"  iy: "+iy+" y:"+a);
    }
}

function initializeVirtualImage()
{
    //calculate();
    calculateHeight();
    PIEremoveElement(virtualObj);
    var loader=new THREE.FontLoader();
    let font=loader.parse(fontJSON);
    virtualObj=new THREE.Mesh(new THREE.TextGeometry(objectAlphabet,{font:font,size:2,height:.1,material:0}),new THREE.MeshLambertMaterial({color:0XFB350B ,transparent: true, opacity: 0.55}));
    virtualObj.scale.set(height,height,1);

    virtualObj.position.x=ix;
    virtualObj.position.z=height;
    virtualObj.position.y=iy-height*2;
    virtualObj.rotation.y+=Math.PI/2;
    PIEaddElement(virtualObj);
    PIErender();

}
var addedvirtual=false;
var h,stage,phase,c,fl,text1,text2,text3,text4,text5,text6;
function updateExperimentElements(t,dt)
{
    if(scene==1)
    {
        ...
        if(drawC<MAX_POINTS)
        {
            incident2.geometry.setDrawRange(0, drawC);
            incident2.geometry.attributes.position.needsUpdate = true;
            PIErender();

            incident3.geometry.setDrawRange(0, drawC);
            incident3.geometry.attributes.position.needsUpdate = true;
            PIErender();
        }
        else if(drawC<MAX_POINTS*2)
        {
            refracted1.geometry.setDrawRange(0, drawC-MAX_POINTS);
            refracted1.geometry.attributes.position.needsUpdate = true;
            PIErender();

            refracted2.geometry.setDrawRange(0, drawC-MAX_POINTS);
            refracted2.geometry.attributes.position.needsUpdate = true;
            PIErender();
        }
        else if(drawC<(MAX_POINTS*2+MAX_POINTS2))
        {
            if(!addedvirtual) {
                setvarpos();
                calculate();
                calculateHeight();
                initializeVirtualImage();
                addedvirtual=true;
            }
            virtual1.geometry.setDrawRange(0, drawC-MAX_POINTS*2);
            virtual1.geometry.attributes.position.needsUpdate = true;
            PIErender();
            virtual2.geometry.setDrawRange(0, drawC-MAX_POINTS*2);
            virtual2.geometry.attributes.position.needsUpdate = true;
            PIErender();
        }
        else if(drawC>MAX_POINTS*6+MAX_POINTS2)
        {
            initVariables();
            resetExperiment();
            PIEstopAnimation();
    ...
    else
    {
        if(first_learn)
        {
            ...
            phase=1;
            c=0;
            PIEremoveElement(incident2);
            PIEremoveElement(incident3);
            PIEremoveElement(refracted1);
            PIEremoveElement(refracted2);
            PIEremoveElement(virtual1);
            PIEremoveElement(virtual2);
            ...
            var loader=new THREE.FontLoader();
            let font=loader.parse(fontJSON);
            virtual_learn=new THREE.Mesh(new THREE.TextGeometry(objectAlphabet,{font:font,size:2,height:.1,material:0}),new THREE.MeshLambertMaterial({color:0XFB350B ,transparent: true, opacity: 0.55}));
            virtual_learn.scale.set(height,height,1);
            text1=new THREE.Mesh(new THREE.TextGeometry(string1[0],{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            text2=new THREE.Mesh(new THREE.TextGeometry(string1[1],{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            text3=new THREE.Mesh(new THREE.TextGeometry(string1[2],{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            text4=new THREE.Mesh(new THREE.TextGeometry(string1[3]+",",{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            text5=new THREE.Mesh(new THREE.TextGeometry("which is just opposite with",{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            text6=new THREE.Mesh(new THREE.TextGeometry("the change of focal length.",{font:font,size:.5,height:.01,material:0}),new THREE.MeshLambertMaterial({color:0XFFFAFA ,transparent: true}));
            ...
            virtual_learn.position.x=U*(-1)*1.5;
            virtual_learn.position.z=h;
            virtual_learn.position.y=-h;
            virtual_learn.rotation.y+=Math.PI/2;
            PIEaddElement(virtual_learn);
            PIErender();
            first_learn=false;
        }
        if(count==0 && phase==1)
        {
            console.log(U);
            benchhandels.position.x=U;
            virtual_learn.position.z=h;
            virtual_learn.position.x=U*(-1)*2.5;
            virtual_learn.scale.y=h;
            virtual_learn.scale.x=h;
            if(U>5) {
                stage = 2;
                c++;
            }
        ...
        else if(count==0 && phase==2)
        {
            benchhandels.position.x=2;
            virtual_learn.position.z=h;
            virtual_learn.position.x=U*(-1)*2.5;
            virtual_learn.scale.y=h;
            virtual_learn.scale.x=h;
        ...
        if(c==6)
        {
            ...
            PIEremoveElement(virtual_learn);
            PIEstopAnimation();
            U=1.5;
            lens.scale.z=0.75;
        }
...
function setObjectDistance(distance)
{
    U=distance;
    ...
    benchhandels.position.x=U;
    PIErender();
    //consolee.log(distance);
    focus2.position.x=distance*3;
    switch(distance)
    {
        case 1.0:
            MAX_POINTS=14;
...
{
    document.title="Virtual image formed with a convex lens";
    PIEsetExperimentTitle("Virtual image by Convex Lens");
    PIEsetDeveloperName("Indranil Biswas");
    //consolee.log("U"+U)
    PIEaddInputCheckbox("Learning",false,learning);
    PIEaddInputCheckbox("Try it Yourself",true,tryYourself);
    PIEaddInputSlider("Object Distance", U, setObjectDistance, 1.5, 4, 0.25);
    PIEaddInputSlider("Focal Length", 4, setFocalLength,2,12,1);// 1.5, 4, .25);


    F=2;

    if(window.outerHeight*1.5>window.innerWidth)
        PIEsetAreaOfInterest(-30,30, 22, -22);
    else
    {
...
var opticalCenter,focus,focus2;
function loadPoints()
{
    opticalCenter=new THREE.Mesh(new THREE.SphereGeometry(0.05,32,32),new THREE.MeshBasicMaterial({color:0xff0000}));
   /* PIEaddElement(opticalCenter);
    opticalCenter.position.x=lens.position.x;
*/
   opticalCenter.position.y+=0.005;
   lens.add(opticalCenter)
    focus=new THREE.Mesh(new THREE.SphereGeometry(0.1,32,32),new THREE.MeshBasicMaterial({color:0xfffd00}));
    PIEaddElement(focus);
    focus.position.x=-0.25;
    focus.position.y+=0.5;

    focus2=new THREE.Mesh(new THREE.SphereGeometry(0.1,32,32),new THREE.MeshBasicMaterial({color:0xfffd00}));
    PIEaddElement(focus2);
    focus2.position.x=6;
    focus2.position.y+=0.5;
    //focus.position.x=lens.position.x;
}
function change_Maxpos()
{
    var a=Math.abs(ix);
    MAX_POINTS2=32+a*10;
}
...
function loadRays()
{
    ...
    var materialLightYellow = new THREE.LineBasicMaterial({color: 0xF4D03F, linewidth: 22});
    var materialLightOrange = new THREE.LineBasicMaterial({color: 0x06F3F9, linewidth: 22});
    var geometry1 = new THREE.BufferGeometry();
    var position1 = new Float32Array( MAX_POINTS * 3 )
    geometry1.addAttribute( 'position', new THREE.BufferAttribute( position1, 3 ) );
    geometry1.setDrawRange( 0, drawC);
    incident1=new THREE.Line(geometry1,materialLightOrange);
    PIEaddElement(incident1);
    PIErender();

    var geometry2 = new THREE.BufferGeometry();
    var position2 = new Float32Array( MAX_POINTS * 3 )
    geometry2.addAttribute( 'position', new THREE.BufferAttribute( position2, 3 ) );
    geometry2.setDrawRange( 0, drawC);
    incident2=new THREE.Line(geometry2,materialLightYellow);
    PIEaddElement(incident2);
    PIErender();

    var geometry3 = new THREE.BufferGeometry();
    var position3 = new Float32Array( MAX_POINTS * 3 )
    geometry3.addAttribute( 'position', new THREE.BufferAttribute( position3, 3 ) );
    geometry3.setDrawRange( 0, drawC);
    incident3=new THREE.Line(geometry3,materialLightYellow);
    PIEaddElement(incident3);
    PIErender();

    var geometry4 = new THREE.BufferGeometry();
    var position4 = new Float32Array( MAX_POINTS * 3 )
    geometry4.addAttribute( 'position', new THREE.BufferAttribute( position4, 3 ) );
    geometry4.setDrawRange( 0, drawC);
    refracted1=new THREE.Line(geometry4,materialLightYellow);
    PIEaddElement(refracted1);
    PIErender();

    var geometry5 = new THREE.BufferGeometry();
    var position5 = new Float32Array( MAX_POINTS * 3 )
    geometry5.addAttribute( 'position', new THREE.BufferAttribute( position5, 3 ) );
    geometry5.setDrawRange( 0, drawC);
    refracted2=new THREE.Line(geometry5,materialLightYellow);
    PIEaddElement(refracted2);
    PIErender();

    var geometry6 = new THREE.BufferGeometry();
    var position6 = new Float32Array( MAX_POINTS2 * 3 *2)
    geometry6.addAttribute( 'position', new THREE.BufferAttribute( position6, 3 ) );
    geometry6.setDrawRange( 0, drawC);
    virtual1=new THREE.Line(geometry6,materialLightYellow);
    PIEaddElement(virtual1);
    PIErender();

    var geometry7 = new THREE.BufferGeometry();
    var position7 = new Float32Array( MAX_POINTS2 * 3*2 )
    geometry7.addAttribute( 'position', new THREE.BufferAttribute( position7, 3 ) );
    geometry7.setDrawRange( 0, drawC);
    virtual2=new THREE.Line(geometry7,materialLightYellow);
    PIEaddElement(virtual2);
    PIErender();
...
function updatePosincident1()
{
    var positions=incident1.geometry.attributes.position.array;
    var x = 0;
    var y = -3.2;
    var z =0;
    var index = 0;
...
function updatePosincident2()
{
    var positions=incident2.geometry.attributes.position.array;
    var x = 0;
    var y = 0+1.5;
    var z =0;
    var index = 0;
...
var dx=0.1,dy=0.025;
function updatePosincident3()
{
    var positions=incident3.geometry.attributes.position.array;
    var x = 0;
    var y = 1.5;
    var z =0;
    var index = 0;
...
function updatePosref1()
{
    var positions=refracted1.geometry.attributes.position.array;
    var x = posxi3;
    var y = posyi3;
    var z =0;
    var index = 0;
...
function updatePosref2()
{
    var positions=refracted2.geometry.attributes.position.array;
    var x = posxi2;
    var y = posyi2;
    var z =0;
    var index = 0;
...
function updatePosvirtual1()
{
    var positions=virtual1.geometry.attributes.position.array;
    var x = posxi3;
    var y = posyi3;
    var z =0;
    var index = 0;

    for ( var i = 0, l = MAX_POINTS2*2; i < l; i ++ ) {
...
function updatePosvirtual2()
{
    var positions=virtual2.geometry.attributes.position.array;
    var x = posxi2;
    var y = posyi2;
    //consolee.log("b")
    var z =0;
...
var ix,dy1=0.02,dx1=-0.1,dy2=0.025,dx2=-0.1,m1=(-0.02/0.1),m2=(-0.05/0.1);
//dy2=0.025;
function calculate()
{
    dy2=dy;
    dx2=-dx;
    //consolee.log("dy2"+dy2);

    var a=posxi2,b=posyi2,m1=(dy1/dx1);
    var c=posxi3,d=posyi3,m2=(dy2/dx2);
    ix=(-b+d+m1*a-m2*c)/(m1-m2);
    //consolee.log("ix"+ix+"m1"+m1+"m2"+m2);
    iy=((m1+m2)*ix-(m1*a+m2*c)+(b+d))/2;
    //consolee.log(m1,m2);
}
...
{
    var mat2=new THREE.MeshPhongMaterial( { color: 0x25E8FA, specular: 0xff0000, shininess: 100, transparent: true, opacity: 0.7 } );
    lens=new THREE.Mesh(new THREE.SphereGeometry( 0.5, 32, 32 ),mat2)
        lens.scale.x=6;
        lens.scale.y=6;
        lens.position.z=0;
        lens.position.x=0;
        ...
        benchhandels=new THREE.Mesh(new THREE.CylinderGeometry(0.25,0.25,2,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
        var cylinder1=new THREE.Mesh(new THREE.CylinderGeometry(0.25,0.25,2,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
        benchhandels.add(cylinder1);
        cylinder1.rotation.x=Math.PI/2;
        cylinder1.position.y=-1;

        var cylinder2=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,1.5,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
        benchhandels.add(cylinder2);
        cylinder2.position.z=-1;
        cylinder2.position.y=-1.5;

        var cylinder3=new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,1.5,32),new THREE.MeshStandardMaterial({color:0x010606,emissive:0x010606,metalness:1}));
        benchhandels.add(cylinder3);
        cylinder3.position.z=1;
        cylinder3.position.y=-1.5;

        benchhandels.position.y=-2.5;
        benchhandels.position.z=0;
        benchhandels.add(lens);
        lens.position.y=3.1;
        benchhandels.position.x=U

        PIEaddElement(benchhandels);
        PIErender();
    }
function lensDragHandle(obj,newpos)
{
...
function initializeText()
{
    ...
    //var s="bla"
    information.innerHTML = "Click&Drag to adjust the camera angle<br/> Scroll to zoom (After clicking on start).";
...
function calcslopeOC() {
    dxoc=0.1;
    dyoc=(0.25/(U-F))*dxoc*-1;
    //consolee.log("slope : "+dyoc+","+dxoc)
}
...
function resetExperiment()
{
    if(scene==1)
    {
        ...
        initVariables();
        loadRays();
        opticalCenter.visible=true;
        focus2.visible=true;
        focus.visible=true;
        PIEremoveElement()
        /*PIEcamera.position.x=0;
        PIEcamera.position.y=0;
        PIEcamera.position.z=14;*/
...
{
    helpContent="";
    helpContent = helpContent + "<h2>Virtual Image formed with Convex Lens(<i>Help</i>)</h2>";
    helpContent = helpContent + "<h3>Help with controls of the experiment</h3>";
    helpContent = helpContent + "<p>On the right top rand side :</p>";
    helpContent = helpContent + "<p>There are 2 sliders and an input text field</p>";
    helpContent = helpContent + "<p>Slide the slider to control the focal length of the lens</p>";
    helpContent = helpContent + "<p>Slide the other slider to control the object distance</p>";
    helpContent = helpContent + "<p>The input text field is to add <i>Analphabet</i></p>";
    helpContent = helpContent + "<p><br/>Check the Learning to demonstrate learning about the experiment</p>";
    helpContent = helpContent + "<p>The try yourself part of the experiment is the part of the activity where you perform the activity</p>";
    helpContent = helpContent + "<p><br/><br/><b>Please set the correct object distance and focal length before starting the experiment</b></p>";
...
{
    helpContent="";
    helpContent = helpContent + "<h2>Virtual Image formed with Convex Lens</h2>";
    helpContent = helpContent + "<h3>About the experiment</h3>";
    helpContent = helpContent + "<p>This experiment helps visualize the virtual image formed with a convex lens.</p>";
    helpContent = helpContent + "<p>This shows how the virtual image is formed with the change of focal length and object distance.</p>";
    helpContent = helpContent + "<p>The focal Length as shown in the changing slider is not the measurement in actual world values .</p>";
    helpContent = helpContent + "<p><br/><br/><b>Please set the correct object distance and focal length before starting the experiment</b></p>";
    helpContent = helpContent + "<p><br/><br/><b>The experiment is conducted to as the increase in  object distance ,less than focal length , increases the image distance and the size.</b></p>";
    helpContent = helpContent + "<p><br/><br/><b>The change of focal also brings the image formed forward for a fixed object .</b><br/></p>";
    helpContent = helpContent + "<p><br/><br/><b>The red point(<img src='reddot.jpg' alt='red dot' width='15' height='15'>) represents the optical center</img></b><br/></p>";
    helpContent = helpContent + "<p><b>The yellow point (<img src='yellow.jpg' alt='yellow dot' width='15' height='15'>) represents the Focus</img></b><br/></p>";
    helpContent = helpContent + "<p>Happy Experimenting. :)</p>";
    PIEupdateInfo(helpContent);
}

var posTobePlaced=1;
function lensInteraction()
{

    // if(!first_learn) {
        if (U == F) {
            alert("Focal length and object are equal !\nImage will be formed at infinity.");
            setFocalLength(4);
            setObjectDistance(1.5);
            PIEchangeInputSlider("Object Distance", 1.5);
            PIEchangeInputSlider("Focal Length", 4);
            resetExperiment();
            PIEstopAnimation()
        }
        else if (U > F) {
            alert("Object distance is greater than the focal length!\nReal Image will be formed.");
            setFocalLength(4);
            setObjectDistance(1.5);
            PIEchangeInputSlider("Object Distance", 1.5);
            PIEchangeInputSlider("Focal Length", 4);
            ...
            posTobePlaced = Math.floor((10 / count) * count2);
        }
    // }
}
