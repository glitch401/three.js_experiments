...
{
    var loader=new THREE.JSONLoader().load('models/opticalbench.json',handle_load);
    function handle_load(geometery,materials)
    {
        var material=new THREE.MeshFaceMaterial(materials);
        var bench=new THREE.Mesh(geometery,material);
        bench.position.y=-6;
        bench.rotation.y+=Math.PI;
        bench.scale.set(3.5,1.75,1.75);
        PIEaddElement(bench);
        PIErender();
    }
}
...
function initializeObject()
{
    ...
    var loader=new THREE.JSONLoader().load('models/lenshandel.json',handle_load2);
    function handle_load2(geometery,materials)
    {
        var material=new THREE.MeshFaceMaterial(materials);
        objecthandle=new THREE.Mesh(geometery,material);
        objecthandle.position.y=-4.5;
        objecthandle.position.z=0;
        object.position.y+=2.8;
        object.position.z+=1.5;
        objecthandle.scale.set(1,1,1);
...
function calculateHeight()
{
    if(firstheight)
    {
        ...
        if(iy>a)
            ...
            height=(a/iy+a%iy);
        console.log("height :"+height+"  iy: "+iy+" y:"+a+"first:"+firstheight);
        firstheight=false;
    }
    else
    {
        ...
        if(iy>a)
            ...
            height=(a/iy+a%iy);
        console.log("height :"+height+"  iy: "+iy+" y:"+a);
    }
}
...
function loadExperimentElements()
{
    ...
    PIEaddInputSlider("Focal Length", 2.5, setFocalLength, 1.5, 4, .25);
    F=2.5;
...
function initializeLens()
{
    ...
    {
        var material=new THREE.MeshFaceMaterial(materials);
        benchhandels=new THREE.Mesh(geometery,material);
        benchhandels.position.y=-4.5;
        benchhandels.position.z=0;
        benchhandels.scale.set(1,1 ,1);
        benchhandels.add(lens);
        benchhandels.position.x=U
        lens.position.y+=5;
        //benchhandels.scale.set(2);
        PIEaddElement(benchhandels);
        PIErender();
       // PIEdragElement(benchhandels);
        //PIEsetDrag(benchhandels,lensDragHandle)
    }
...
function initializeText()
{
    ...
    information.style.color = '#F90606';
    ...
    //var s="bla"
    information.innerHTML = "Drag lens to move it and scroll to zoom (After clicking on start)<br/>To start the experiment properly <br />  click & set the object distance & focal length sliders";
...
function lensInteraction()
{
    ...
    if(U==F)
    {
        ...
        PIEchangeInputSlider("Focal Length",2.5);
        resetExperiment();
        PIEstopAnimation()
    }
    else if(U>F)
